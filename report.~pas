...
const name_glv:array[0..1]of string[20]=('Параметр','Дата/Время/Единицы');
      str_evt:array[0..4]of string[30]=('Дата и время','Старое знач',
             'Новое знач','Параметр/список или действие','Идентифик');
...
begin
  ...
  sgArchive.ColWidths[0]:=101;size:=0;ClearTable(sgArchive);
  ClearTable(sgEvent);ClearTable(sgAlarm);
  ...
  sgEvent.ColWidths[3]:=181;sgEvent.ColWidths[4]:=55;
  sgAlarm.ColWidths[1]:=151;sgAlarm.ColWidths[3]:=191;
  ...
  begin
    ...
    sgArchive.Cells[size+1,1]:=units_report[i];size:=size+1;
  end;max_col:=size;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FloatToStrF(a_data[i],ffGeneral,6,5);size:=size+1;
        end;num_str:=num_str+1;
      end;num_rec:=num_rec+1;
    end;CloseFile(f_arc);sgArchive.RowCount:=num_str+sgArchive.FixedRows+1;
  end;max_data:=num_str;lCountData.Caption:=IntToStr(max_data);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        sgEvent.Cells[4,num_str+sgEvent.FixedRows]:='Коммуник' else
        if ind_ident=16 then sgEvent.Cells[4,num_str+sgEvent.FixedRows]:='Терминал';
        num_str:=num_str+1;
      end;num_rec:=num_rec+1;
    end;CloseFile(f_event);sgEvent.RowCount:=num_str+sgEvent.FixedRows+1;
  end;max_event:=num_str;lCountEvent.Caption:=IntToStr(max_event);
  num_rec:=0;num_str:=0;
  ...
  begin
    ...
    end;CloseFile(f_event);sgAlarm.RowCount:=num_str+sgAlarm.FixedRows+1;
  end;max_alarm:=num_str;lCountAlarm.Caption:=IntToStr(max_alarm);
end;
//---------------------------------------------------//
...
begin
  ...
  begin
    FormForVariablesView.StrToStrings(Cells[ACol, ARow]);Canvas.FillRect(Rect);
